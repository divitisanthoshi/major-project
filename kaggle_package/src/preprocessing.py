...
def attention_weighted_motion(
    sequence: np.ndarray,
    joint_weights: np.ndarray,
    last_n_frames: int = 16,
) -> float:
    """
    Motion energy weighted by joint attention: when attended body parts move, return higher value.
    Used to gate quality score and rep counting so they increase with motion in important joints.

    Args:
        sequence: (T, V, C) e.g. (64, 33, 3).
        joint_weights: (V,) attention weight per joint from model (sum to 1).
        last_n_frames: Use only last N frames for recent motion.

    Returns:
        Scalar >= 0; higher when high-attention joints move more.
    """
    if sequence is None or sequence.shape[0] < 2 or joint_weights is None:
        return 0.0
    T, V, C = sequence.shape
    n = min(last_n_frames, T - 1)
    if n < 1 or joint_weights.size != V:
        return 0.0
    w = np.reshape(joint_weights, (1, -1, 1)).astype(np.float64)  # (1, V, 1)
    tail = sequence[-n - 1 :]  # (n+1, V, C)
    diffs = np.diff(tail, axis=0)  # (n, V, C)
    frame_delta = np.sqrt(np.sum(diffs ** 2, axis=2))  # (n, V)
    weighted = frame_delta * np.squeeze(w)  # (n, V)
