...
from src.models.layers import SpatialGraphConv, TemporalConv, ReduceSum, JointAttention
...
def build_rehab_grading_model(
    ...
    # Joint attention: weight joints by importance (interpretable; drives quality/reps from attended body parts)
    joint_att = JointAttention(units=attention_units)
    joint_att_out = joint_att(x)
    x = joint_att_out[0]   # weighted (B, T, V, C)
    joint_weights = joint_att_out[1]  # (B, V) for use in attention-weighted motion
    ...
    # Temporal attention over LSTM steps
    ...
    x = layers.Dropout(dropout)(x)
    score_out = layers.Dense(1, activation="sigmoid")(x)

    # Two outputs: score (for loss/training) and joint_weights (for attention-weighted motion at inference)
    model = keras.Model(inputs, [score_out, joint_weights], name="RehabGradingSTGCN")
    # Compile is done in train.py so loss/metrics match multi-output correctly
    return model
